# 버전 2 - 출력부 TextInput(복사/편집 가능) + 에러 표시 복구 + 입력칸 여백 축소
# -*- coding: utf-8 -*-
...
class DigitInput(TextInput):
    ...
    def __init__(self, **kw):
        ...
        self.halign = "left"
        self.padding = (dp(4), dp(6))   # 내부 여백 좀 더 축소
        self.font_name = FONT
    ...
    def insert_text(self, substring, from_undo=False):
        if self.allow_float:
            ...
            if "." in self.text and "." in f: f = f.replace(".", "")
        else:
            f = "".join(ch for ch in substring if ch.isdigit())
        remain = max(0, self.max_len - len(self.text))
        if remain <= 0: return
        if len(f) > remain: f = f[:remain]
        return super().insert_text(f, from_undo=from_undo)
...
class SlabApp(App):
    ...
    def build(self):
        ...
        # 제목
        title_row = BoxLayout(size_hint=(1, None), height=dp(42))
        ...
        row_code.add_widget(Label(text="강번 입력:", font_name=FONT, color=(0,0,0,1),
                                  size_hint=(None,1), width=dp(72), halign="right", valign="middle"))
        row_code.add_widget(Label(text=self.prefix, font_name=FONT, color=(0,0,0,1),
                                  size_hint=(None,1), width=dp(46), halign="left", valign="middle"))
        self.in_code_front = DigitInput(max_len=3, size_hint=(None,1), width=dp(58))
        ...
        self.in_code_back = DigitInput(max_len=1, size_hint=(None,1), width=dp(38))
        ...
        self.in_total = DigitInput(max_len=5, allow_float=True, size_hint=(None,1), width=dp(118))
        row_total.add_widget(self.in_total)
        row_total.add_widget(Widget())
        root.add_widget(row_total)

        # 지시길이 + 복사 버튼 (간격 고정)
        grid = GridLayout(cols=4, size_hint=(1, None), height=dp(28*3 + 6*2),
                          row_default_height=dp(28), row_force_default=True, spacing=dp(6))

        self.in_p1 = DigitInput(max_len=4, allow_float=True, size_hint=(None,1), width=dp(96))
        self.in_p2 = DigitInput(max_len=4, allow_float=True, size_hint=(None,1), width=dp(96))
        self.in_p3 = DigitInput(max_len=4, allow_float=True, size_hint=(None,1), width=dp(96))
        ...
        # 출력부: 복사/붙여넣기 가능(TextInput). 기본은 읽기전용이지만, 편집 가능하게 원하셔서 False로 둠.
        self.result_box = TextInput(text="",
                                    readonly=False,   # 손으로 수정도 가능
        ...
        # 버전 표기
        ver = Label(text="버전 2", font_name=FONT, color=(0.4,0.4,0.4,1),
                    halign="right", valign="middle", size_hint=(1, None), height=dp(20))
        ver.bind(size=lambda *_: setattr(ver, "text_size", ver.size))
        root.add_widget(ver)
    ...
    def _err(self, msg):
        # warning.png는 결과창엔 삽입할 수 없으니 텍스트로 표시
        self.result_box.text = f"⚠️ {msg}"
    ...
    def calculate(self, *_):
        try:
            ...
            if slab is None or slab <= 0:
                self._err("실제 Slab 길이를 올바르게 입력하세요.")
                return

            guides = [v for v in (p1, p2, p3) if v is not None and v > 0]
            if len(guides) < 2:
                self._err("최소 2개 이상의 지시길이를 입력하세요.")
                return
