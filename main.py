...
class DigitInput(TextInput):
    ...
    def __init__(self, **kwargs):
        ...
        self.halign = "left"; self.padding = (dp(6), dp(5))
        self.font_name = FONT; self.font_size = dp(16); self.height = dp(30)
        self.background_normal = ""; self.background_active = ""; self.cursor_width = dp(2)
...
class AlnumInput(TextInput):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.multiline = False; self.halign = "left"; self.padding = (dp(6), dp(5))
        self.font_name = FONT; self.font_size = dp(16); self.height = dp(30)
    ...
    def _apply_settings_from_file(self):
        if os.path.exists(SETTINGS_FILE):
            try:
                ...
                size = 11
                try: size = int(str(s.get("font", "11")))
                except: size = 11
                self.out_box.font_size = dp(size)
                self._round_int = bool(s.get("round", False))
                self._remove_mm = bool(s.get("remove_mm", False))
                self._loss_override = s.get("loss", None)
            except: pass
        else:
            self._round_int = False; self._remove_mm = False; self._loss_override = None
    ...
    def calculate(self, *_):
        try:
            ...
            if slab is None or slab <= 0:
                self.out_box.text = ""; self._show_warn("실제 Slab 길이를 올바르게 입력하세요."); return
            guides = [v for v in (p1, p2, p3) if v is not None and v > 0]
            if len(guides) < 2:
                self.out_box.text = ""; self._show_warn("최소 2개 이상의 지시길이를 입력하세요."); return
            self._hide_warn()
            ...
            def fmt(x):
                return f"{round_half_up(x):,d}" if self._round_int else f"{x:,.1f}"
            ...
            for i, g in enumerate(guides, 1): lines.append(f"▶ {i}번 지시길이: {fmt(g)}{mm}")
            ...
            for i, r in enumerate(real, 1): lines.append(f"   {i}번: {fmt(r)}{mm}")
            visual = "H"
...
class SettingsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        hint = Label(text="출력부 소수점 값을 정수로 표시", font_name=FONT, color=(0.5,0.5,0.5,1),
                     halign="left", valign="middle")
        hint.bind(size=lambda *_: setattr(hint, "text_size", hint.size))
        row2.add_widget(self.chk_round); row2.add_widget(hint); row2.add_widget(Widget())
    ...
    def _save_and_back(self):
        ...
        self.manager.get_screen("main")._apply_settings_from_file()
...
class SlabApp(App):
    def build(self):
        ...
        def current_switch(to_name): sm.current = to_name
        sm.current_switch = current_switch
        return sm
