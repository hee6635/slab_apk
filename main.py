...
# ---------------- 둥근 버튼 (Button 기반, Label처럼 보이도록 고정) ----------------
...
class SlabApp(App):
    ...
    def build(self):
        ...
        # 0) 헤더: 우상단 '설정' 고정
        header = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(40))
        header.add_widget(Widget(size_hint_x=1))  # 우측 정렬용 스페이서
        ...
        # 1) 타이틀: 그 아래 중앙 정렬
        title_row = BoxLayout(orientation="horizontal", size_hint=(1, None), height=dp(44))
        title_row.add_widget(Widget(size_hint_x=1))
        title = Label(text="후판 계산기", font_name=FONT, font_size=dp(28),
                      color=(0, 0, 0, 1), halign="center", valign="middle",
                      size_hint=(None, 1))
        title.bind(size=lambda *_: setattr(title, "text_size", title.size))
        # 텍스트 폭을 내용에 맞게 조금 여유롭게
        title.width = dp(180)
        ...
        row_total.add_widget(Widget(size_hint=(1,1)))
        root.add_widget(row_total)

        # 4) 지시길이 (4자리) + 복사 버튼 (간격 고정)
        grid = GridLayout(cols=4, size_hint=(1, None), height=dp(32*3+8*2),
                          row_default_height=dp(32), row_force_default=True, spacing=dp(8))

        def _mk_label(txt, w=110):
            lb = Label(text=txt, font_name=FONT, color=(0,0,0,1),
                       size_hint=(None,1), width=dp(w), halign="right", valign="middle")
        ...
        # 1번
        grid.add_widget(_mk_label("1번 지시길이:"))
        grid.add_widget(self.in_p1)
        grid.add_widget(Label())
        grid.add_widget(Label())

        # 2번 (+ 고정 간격 버튼 묶음)
        grid.add_widget(_mk_label("2번 지시길이:"))
        grid.add_widget(self.in_p2)
        btn_row2 = BoxLayout(orientation="horizontal", size_hint=(None,1),
                             width=dp(140), spacing=dp(8))
        ...
        btn_row2.add_widget(b21)
        # 빈 자리(2열은 버튼 1개만)
        btn_row2.add_widget(Widget(size_hint=(None,1), width=dp(72)))
        grid.add_widget(btn_row2)
        grid.add_widget(Label())

        # 3번 (+ 고정 간격 버튼 2개)
        grid.add_widget(_mk_label("3번 지시길이:"))
        grid.add_widget(self.in_p3)
        btn_row3 = BoxLayout(orientation="horizontal", size_hint=(None,1),
                             width=dp(140), spacing=dp(8))
        ...
        btn_row3.add_widget(b31); btn_row3.add_widget(b32)
        grid.add_widget(btn_row3)
        grid.add_widget(Label())
    ...
    def open_settings(self, *_):
        ...
        msg = Label(text="설정은 추후 추가 예정입니다.", font_name=FONT, color=(0,0,0,1))
        box.add_widget(msg)
