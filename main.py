# -*- coding: utf-8 -*-
# 버전 18R - 여백/폭/폰트/스위치/표기/포맷 전면 정리 (안정 빌드)
...
class DigitInput(TextInput):
    ...
    def __init__(self, **kwargs):
        ...
        self.font_name = FONT
        self.font_size = dp(16)   # 입력박스 폰트 통일
        self.height = dp(30)
...
class MainScreen(Screen):
    # 입력 폭 (최신 합의값)
    CODE_FRONT_W = dp(48)   # 3자리
    CODE_BACK_W  = dp(26)   # 1자리
    SLAB_W       = dp(74)   # 5자리
    GUIDE_W      = dp(66)   # 4자리
    ...
    def build_ui(self):
        ...
        self.in_code_front = DigitInput(max_len=3, allow_float=False, width=self.CODE_FRONT_W)
    ...
    def calculate(self):
        try:
            ...
            # ⑦: ON이면 '절단 예상 길이'를 맨 아래로(= 시각화 먼저, 그다음 예상 길이)
...
class SettingsScreen(Screen):
    ...
    def _title(self, text):
        lab = Label(text=text, font_name=FONT, font_size=dp(32),   # 크게
                    color=(0,0,0,1), halign="center", valign="middle",
    ...
    def build_ui(self):
        ...
        self.ed_prefix = AlnumInput(max_len=4, width=self.BASE_W)  # 4자리 제한 + 70dp
        ...
        self.ed_out_font = DigitInput(max_len=2, allow_float=False, width=self.TWO_W)
        try:
            self.ed_out_font.text = str(int(self.app.st.get("out_font", 15)))
        except Exception:
            self.ed_out_font.text = "15"
        row3.add_widget(self.ed_out_font)
        row3.add_widget(self._gray("결과 표시 라벨 폰트 크기", h=dp(30)))
        ...
        row4.add_widget(self._gray("단위(mm) 문구 숨김", h=dp(30)))
        body.add_widget(row4)

        # 5. 절단 손실 길이 조정
        body.add_widget(self._leftlab("5. 절단 손실 길이 조정"))
        row5 = BoxLayout(size_hint=(1,None), height=dp(30), spacing=dp(8))
        self.ed_loss = DigitInput(max_len=2, allow_float=True, width=self.TWO_W)  # 2자리 45dp
        loss_def = float(self.app.st.get("loss_mm", 15.0))
        self.ed_loss.text = f"{loss_def:.0f}"
        ...
        row6.add_widget(self._gray("해상도에 맞게 입력부 폰트 조절", h=dp(30)))
        ...
        row7.add_widget(self._gray("ON 시 '절단 예상 길이'를 맨 아래로", h=dp(30)))
